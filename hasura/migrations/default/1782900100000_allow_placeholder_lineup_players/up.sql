...
BEGIN
    IF
        match_lineup_player.steam_id IS NULL
        AND match_lineup_player.discord_id IS NULL
        AND match_lineup_player.placeholder_name IS NULL
    THEN
        RAISE EXCEPTION USING ERRCODE = '22000', MESSAGE = 'steam_id, discord_id or placeholder_name is required';
    END IF;
    ...
    IF match_lineup_player.steam_id IS NOT NULL AND EXISTS (
        SELECT 1
    ...
    IF match_lineup_player.captain = true AND match_lineup_player.steam_id IS NOT NULL THEN
